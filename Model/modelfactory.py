...
class ModelFactory:
	...
	def createModel(self, mType):
		if mType == ModelType.WAGES:
			model = WageModel(dsb.getModelData(ModelType.WAGES))
		else if mType == ModelType.INVESTS:
			model = InvestmentModel(dsb.getModelData(ModelType.INVESTS))
		else if mType == ModelType.ASSETS:
			model = AssetModel(dsb.getModelData(ModelType.ASSETS))
		else:
